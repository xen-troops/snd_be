...
unordered_map<int, CommandHandler::CommandFn> CommandHandler::sCmdTable = {
    {XENSND_OP_OPEN, &CommandHandler::open},       {XENSND_OP_CLOSE, &CommandHandler::close},
    {XENSND_OP_READ, &CommandHandler::read},       {XENSND_OP_WRITE, &CommandHandler::write},
    {XENSND_OP_TRIGGER, &CommandHandler::trigger}, {XENSND_OP_HW_PARAM_QUERY, &CommandHandler::queryHwParam},
};
...
CommandHandler::CommandHandler(PcmDevicePtr pcmDevice, EventRingBufferPtr eventRingBuffer, domid_t domId)
    : mPcmDevice(pcmDevice), mDomId(domId), mEventRingBuffer(eventRingBuffer), mEventId(0), mLog("CommandHandler")
{
...
int CommandHandler::processCommand(const xensnd_req& req, xensnd_resp& rsp)
{
    ...
    try {
        ...
        if (status >= 0) {
            DLOG(mLog, WARNING) << "Positive error code: " << static_cast<signed int>(status);
...
void CommandHandler::open(const xensnd_req& req, xensnd_resp& rsp)
{
    ...
    mPcmDevice->open(
        {openReq.pcm_rate, openReq.pcm_format, openReq.pcm_channels, openReq.buffer_sz, openReq.period_sz});
}
...
void CommandHandler::read(const xensnd_req& req, xensnd_resp& rsp)
{
    ...
    mPcmDevice->read(&(static_cast<uint8_t*>(mBuffer->get())[readReq.offset]), readReq.length);
}
...
void CommandHandler::write(const xensnd_req& req, xensnd_resp& rsp)
{
    ...
    mPcmDevice->write(&(static_cast<uint8_t*>(mBuffer->get())[writeReq.offset]), writeReq.length);
}
...
void CommandHandler::getBufferRefs(grant_ref_t startDirectory, uint32_t size, vector<grant_ref_t>& refs)
{
    refs.clear();

    size_t requestedNumGrefs = (size + XC_PAGE_SIZE - 1) / XC_PAGE_SIZE;

    DLOG(mLog, DEBUG) << "Get buffer refs, directory: " << startDirectory << ", size: " << size
    ...
    while (startDirectory != 0) {
        ...
        size_t numGrefs =
            min(requestedNumGrefs, (XC_PAGE_SIZE - offsetof(xensnd_page_directory, gref)) / sizeof(uint32_t));

        DLOG(mLog, DEBUG) << "Gref address: " << pageDirectory->gref << ", numGrefs " << numGrefs;

        refs.insert(refs.end(), pageDirectory->gref, pageDirectory->gref + numGrefs);
