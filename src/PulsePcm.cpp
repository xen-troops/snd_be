...
PulseMainloop::PulseMainloop(const string& name)
    : mMainloop(nullptr), mContext(nullptr), mMutex(nullptr), mLog("PulseMainloop")
{
    try {
        init(name);
    }
...
PulsePcm* PulseMainloop::createStream(StreamType type, const string& name, const string& propName,
                                      const string& propValue, const string& deviceName)
{
    return new PulsePcm(mMainloop, mContext, type, name, propName, propValue, deviceName);
}
...
PulsePcm::PulsePcm(pa_threaded_mainloop* mainloop, pa_context* context, StreamType type, const string& name,
                   const string& propName, const string& propValue, const string& deviceName)
    : mMainloop(mainloop),
      mContext(context),
      mStream(nullptr),
      mTimeEvent(nullptr),
...
void PulsePcm::open(const PcmParams& params)
{
    ...
    LOG(mLog, DEBUG) << "Format: " << pa_sample_format_to_string(convertPcmFormat(params.format))
                     << ", rate: " << params.rate << ", channels: " << static_cast<int>(params.numChannels)
                     << ", period: " << params.periodSize << ", buffer: " << params.bufferSize;
...
void PulsePcm::read(uint8_t* buffer, size_t size)
{
    ...
    while (size > 0) {
        ...
        memcpy(buffer, static_cast<const uint8_t*>(mReadData) + mReadIndex, readableSize);
...
void PulsePcm::write(uint8_t* buffer, size_t size)
{
    ...
    while (size > 0) {
        ...
        if (pa_stream_write(mStream, buffer, writableSize, nullptr, 0LL, PA_SEEK_RELATIVE) < 0) {
            contextError("Can't write stream", mContext);
        }
...
void PulsePcm::updateTimingCbk(int success)
{
    ...
    if (mProgressCbk && !pa_stream_is_corked(mStream)) {
        DLOG(mLog, DEBUG) << "Update timing, usec: " << time / 1000 << ", bytes: " << bytes;
...
int PulsePcm::waitOperationFinished(pa_operation* op)
{
    ...
    while (pa_operation_get_state(op) == PA_OPERATION_RUNNING && (states = getStatus()) == PA_OK) {
        pa_threaded_mainloop_wait(mMainloop);
    }
...
{
    if (!mContext || !PA_CONTEXT_IS_GOOD(pa_context_get_state(mContext)) || !mStream ||
        !PA_STREAM_IS_GOOD(pa_stream_get_state(mStream))) {
...
void PulsePcm::createStream()
{
    ...
    mStream = pa_stream_new_with_proplist(mContext, mName.c_str(), &mSampleSpec, nullptr, &propList);
...
void PulsePcm::connectPlaybackStream(const char* deviceName)
{
    ...
    if (pa_stream_connect_playback(
            mStream, deviceName, &bufferAttr,
            static_cast<pa_stream_flags_t>(PA_STREAM_START_CORKED | PA_STREAM_INTERPOLATE_TIMING |
...
void PulsePcm::connectCaptureStream(const char* deviceName)
{
    ...
    if (pa_stream_connect_record(
            mStream, deviceName, nullptr,
            static_cast<pa_stream_flags_t>(PA_STREAM_INTERPOLATE_TIMING | PA_STREAM_ADJUST_LATENCY |
