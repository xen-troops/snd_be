...
void AlsaPcm::open(const PcmParams& params)
{
    try {
        ...
        snd_pcm_stream_t streamType = mType == StreamType::PLAYBACK ? SND_PCM_STREAM_PLAYBACK : SND_PCM_STREAM_CAPTURE;

        int ret = 0;

        queryClose();

        if ((ret = snd_pcm_open(&mHandle, mDeviceName.c_str(), streamType, 0)) < 0) {
        ...
        mTimerPeriodMs = milliseconds((snd_pcm_bytes_to_frames(mHandle, mParams.periodSize) * 1000) / mParams.rate);
    }
...
{
    DLOG(mLog, DEBUG) << "Read from pcm device: " << mDeviceName << ", size: " << size;
    ...
    while (numFrames > 0) {
        if (auto status = snd_pcm_readi(mHandle, buffer, numFrames)) {
            if (status == -EPIPE) {
                LOG(mLog, WARNING) << "Device: " << mDeviceName << ", message: " << snd_strerror(status);

                snd_pcm_prepare(mHandle);
            }
            else if (status < 0) {
                throw Exception("Read from audio interface failed: " + mDeviceName, -status);
            }
            else {
                numFrames -= status;
...
void AlsaPcm::write(uint8_t* buffer, size_t size)
{
    ...
    while (numFrames > 0) {
        if (auto status = snd_pcm_writei(mHandle, buffer, numFrames)) {
            DLOG(mLog, DEBUG) << "Write to pcm device: " << mDeviceName << ", size: " << status;

            if (status == -EPIPE) {
                LOG(mLog, WARNING) << "Device: " << mDeviceName << ", message: " << snd_strerror(status);

                if ((status = snd_pcm_recover(mHandle, status, 0)) < 0) {
                    throw Exception("Can't recover underrun: " + mDeviceName, -status);
                }
                ...
                throw Exception("Write to audio interface failed: " + mDeviceName, -status);
            }
            else {
                numFrames -= status;
                buffer = &buffer[snd_pcm_frames_to_bytes(mHandle, status)];
...
{
    LOG(mLog, DEBUG) << "Format: " << snd_pcm_format_name(convertPcmFormat(params.format)) << ", rate: " << params.rate
                     << ", channels: " << static_cast<int>(params.numChannels) << ", period: " << params.periodSize
    ...
    if ((ret = snd_pcm_hw_params_set_access(mHandle, hwParams, SND_PCM_ACCESS_RW_INTERLEAVED)) < 0) {
        throw Exception("Can't set access " + mDeviceName, -ret);
    }

    snd_pcm_format_t format = convertPcmFormat(params.format);

    if ((ret = snd_pcm_hw_params_set_format(mHandle, hwParams, format)) < 0) {
        throw Exception("Can't set format " + mDeviceName, -ret);
    }

    if ((ret = snd_pcm_hw_params_set_rate(mHandle, hwParams, params.rate, 0)) < 0) {
        throw Exception("Can't set rate " + mDeviceName, -ret);
    }

    if ((ret = snd_pcm_hw_params_set_channels(mHandle, hwParams, params.numChannels)) < 0) {
    ...
    if (params.bufferSize) {
        bufferFrames = params.bufferSize / (params.numChannels * snd_pcm_format_size(format, 1));
    }

    if ((ret = snd_pcm_hw_params_set_buffer_size_near(mHandle, hwParams, &bufferFrames)) < 0) {
        throw Exception("Can't set buffer size " + mDeviceName, -ret);
    }

    mParams.bufferSize = bufferFrames * (params.numChannels * snd_pcm_format_size(format, 1));

    if (params.bufferSize && params.bufferSize != mParams.bufferSize) {
        LOG(mLog, WARNING) << "Can't set requested buffer size. "
                           << "Nearest value will be used: " << mParams.bufferSize;
    }

    snd_pcm_uframes_t periodFrames = cDefaultPeriodFrames;

    if (params.periodSize) {
        periodFrames = params.periodSize / (params.numChannels * snd_pcm_format_size(format, 1));
    }

    if ((ret = snd_pcm_hw_params_set_period_size_near(mHandle, hwParams, &periodFrames, 0)) < 0) {
        throw Exception("Can't set period size " + mDeviceName, -ret);
    }

    mParams.periodSize = periodFrames * (params.numChannels * snd_pcm_format_size(format, 1));

    if (params.periodSize && params.periodSize != mParams.periodSize) {
        LOG(mLog, WARNING) << "Can't set requested period size. "
                           << "Nearest value will be used: " << mParams.periodSize;
    }
...
void AlsaPcm::setSwParams()
{
    ...
    if ((ret = snd_pcm_sw_params_set_tstamp_type(mHandle, swParams, SND_PCM_TSTAMP_TYPE_MONOTONIC_RAW)) < 0) {
        throw Exception("Can't set ts type " + mDeviceName, -ret);
    }

    snd_pcm_uframes_t threshold = snd_pcm_bytes_to_frames(mHandle, mParams.bufferSize) * 2;

    if ((ret = snd_pcm_sw_params_set_start_threshold(mHandle, swParams, threshold)) < 0) {
...
void AlsaPcm::getTimeStamp()
{
    ...
    uint64_t frame = ((audioTimeStamp.tv_sec * 1000000000 + audioTimeStamp.tv_nsec) * mParams.rate) / 1000000000;
...
void AlsaPcm::queryOpen()
{
    try {
        ...
        if (!mHwQueryHandle) {
            ...
            if ((ret = snd_pcm_open(&mHwQueryHandle, mDeviceName.c_str(), streamType, 0)) < 0) {
                throw Exception("Can't open audio device " + mDeviceName, -ret);
            }
...
void AlsaPcm::queryHwParamRate(snd_pcm_hw_params_t* hwParams, SoundItf::PcmParamRanges& req,
                               SoundItf::PcmParamRanges& resp)
{
    int ret;

    if ((ret = snd_pcm_hw_params_set_rate_minmax(mHwQueryHandle, hwParams, &req.rates.min, 0, &req.rates.max, 0)) < 0) {
...
void AlsaPcm::queryHwParamBuffer(snd_pcm_hw_params_t* hwParams, SoundItf::PcmParamRanges& req,
                                 SoundItf::PcmParamRanges& resp)

{
    ...
    if ((ret = snd_pcm_hw_params_set_buffer_size_minmax(mHwQueryHandle, hwParams, &minFrames, &maxFrames)) < 0) {
...
void AlsaPcm::queryHwParamChannels(snd_pcm_hw_params_t* hwParams, SoundItf::PcmParamRanges& req,
                                   SoundItf::PcmParamRanges& resp)
{
    int ret;

    if ((ret = snd_pcm_hw_params_set_channels_minmax(mHwQueryHandle, hwParams, &req.channels.min, &req.channels.max)) <
        0) {
...
void AlsaPcm::queryHwParamPeriod(snd_pcm_hw_params_t* hwParams, SoundItf::PcmParamRanges& req,
                                 SoundItf::PcmParamRanges& resp)
{
    ...
    if ((ret = snd_pcm_hw_params_set_period_size_minmax(mHwQueryHandle, hwParams, &minFrames, 0, &maxFrames, 0)) < 0) {
...
void AlsaPcm::queryHwParamFormats(snd_pcm_hw_params_t* hwParams, SoundItf::PcmParamRanges& req,
                                  SoundItf::PcmParamRanges& resp)
{
    ...
    if ((ret = snd_pcm_hw_params_set_format_mask(mHwQueryHandle, hwParams, alsa_formats)) < 0) {
